...
if rospy.has_param('~Obj'):
    Obj=rospy.get_param('~Obj')
    collect_idx=rospy.get_param('~collect_idx')
    version=rospy.get_param('~version')

class zs_transition_experience():
    path = '/home/szhang/Desktop/red_data/'
    ...
    def __init__(self, Load=True, discrete = True, postfix='', Object = Obj, with_fingers = False):
        ...
        if postfix == 'bu':
            ...
            self.file_name = self.path + 'zs_raw_' + self.Object + '_' + self.mode + '_v' + version + self.postfix + '.obj'
