...
def check_min_alpha_fn(r):
    x=r.progress['EpisodesSoFar']
    y=r.progress['MinAlphaMean']
    return x,y
def check_max_alpha_fn(r):
    x=r.progress['EpisodesSoFar']
    y=r.progress['MaxAlphaMean']
    return x,y
def check_mean_alpha_fn(r):
    x=r.progress['EpisodesSoFar']
    y=r.progress['MeanAlphaMean']
    return x,y
def check_alpha_percent_fn(r):
    x=r.progress['EpisodesSoFar']
    y=r.progress['ClfAlphaPercentMean']
    return x,y
...
def plot_results(
    ...
    xlabel=None,
    ylabel=None,
    plot_reacher_optimal=True,
    aip_no_regress=False
):
    '''
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        for result in sresults:
            ...
            if average_group:
                ...
                g2l[group] = l
        if aip_no_regress:
            colorss=['blue','cyan','red','black']
        else:
            colorss=['blue','cyan','red','green','black','brown','purple','yellow','orange']
        if average_group:
            idx=0
        ...
        plt.tight_layout()
        if env=='Reacher-v2' and plot_reacher_optimal:
            opt,=plt.plot(-6*np.ones(500000),':',label='optimal')
        if any(g2l.keys()):
            if env=='Reacher-v2' and plot_reacher_optimal:
                kys=copy.copy(g2l)
...
def plot_results_alpha(
    ...
    '''
    if key=='mean':
        xy_fn=check_mean_alpha_fn
    elif key=='min':
        xy_fn=check_min_alpha_fn
    elif key=='max':
        xy_fn=check_max_alpha_fn
    elif key=='percent':
        xy_fn=check_alpha_percent_fn
    if split_fn is None: split_fn = lambda _ : ''
    if group_fn is None: group_fn = lambda _ : ''
    sk2r = defaultdict(list) # splitkey2results
    for result in allresults:
