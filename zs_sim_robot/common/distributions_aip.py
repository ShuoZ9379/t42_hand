...
def reacher_adjust_obs(obs):
    if len(obs.shape)==1:
        ...
        return np.concatenate((obs[:,:4],obs[:,6:8],obs[:,8:10]+obs[:,4:6],obs[:,4:6]))

def get_ppo_action_ref(obs,policy_ref,env_name='Reacher-v2'):
    if env_name=='Reacher-v2':
        adjusted_obs=reacher_adjust_obs(obs)
        actions, values, _, neglogpacs=policy_ref.step(adjusted_obs)
        action_ref=actions.reshape((adjusted_obs.shape[0],-1))
    else:
...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, ref_a, alpha, init_scale=1.0, init_bias=0.0):
        mean = _matching_fc(latent_vector, 'pi', self.size, init_scale=init_scale, init_bias=init_bias)
        logstd = tf.get_variable(name='pi/logstd', shape=[1, self.size], initializer=tf.zeros_initializer())
        pdparam = tf.concat([(1-alpha)*mean+alpha*ref_a, mean * 0.0 + logstd], axis=1)
...
def make_pdtype(env_type, ac_space):
    if env_type!='corl':
