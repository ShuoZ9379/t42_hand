...
try:
    ...
    MPI = None
from ppo2_free.runner_free import Runner
if not os.path.exists('./ppo2_free_results/eval/'):
    os.makedirs('./ppo2_free_results/eval/')
if not os.path.exists('./ppo2_free_results/single_loss/'):
    os.makedirs('./ppo2_free_results/single_loss/')
if not os.path.exists('./ppo2_free_results/test_ah_single_loss_withreachgoal_ctrl1/'):
    os.makedirs('./ppo2_free_results/test_ah_single_loss_withreachgoal_ctrl1/')
...
def learn(*, network, env, env_type, total_timesteps, eval_env = None, need_eval=False, num_eval_eps=1, compare=False,compare_ah_idx=8,reacher_sd=1,acrobot_sd=1,
            ...
            save_interval=1, load_path=None, model_fn=None, update_fn=None, init_fn=None, mpi_rank_weight=1, comm=None, plot_single_loss=False, single_loss_suf='', min_trans=2000, **network_kwargs):
    '''
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from ppo2_free.model_free import Model
        model_fn = Model
    ...
    if not (env_type=='corl' and need_eval==True and eval_env is None):
        ...
        while data_size<total_timesteps:
            #assert nbatch % nminibatches == 0
            update+=1
            # Start timer
            tstart = time.perf_counter()
            #frac = 1.0 - (update - 1.0) / nupdates
            frac = 1.0 - data_size / total_timesteps
            ...
            # Get minibatch
            obs, returns, masks, actions, values, neglogpacs, states, epinfos, final_obs = runner.run(min_trans=min_trans) #pylint: disable=E0632
            if eval_env is not None:
                eval_obs, eval_returns, eval_masks, eval_actions, eval_values, eval_neglogpacs, eval_states, eval_epinfos, eval_final_obs= eval_runner.run(min_trans=min_trans) #pylint: disable=E0632
            ...
            if update % log_interval == 0 or update == 1:
                ...
                logger.logkv("misc/serial_timesteps", data_size*nenvs)
                logger.logkv("misc/nupdates", update)
                #logger.logkv("misc/total_timesteps", update*nbatch)
                logger.logkv("misc/total_timesteps", data_size)
        ...
        if plot_single_loss and env_type=='corl':
            ...
            plt.savefig('./ppo2_free_results/test_ah_single_loss'+single_loss_suf+'/'+env.env_name+'_single_seed_'+str(seed)+suf+'_loss.png')
        ...
        for eval_eps in range(num_eval_eps):
            ...
            obs, returns, masks, actions, values, neglogpacs, states, epinfos, final_obs = runner.run(do_eval=True,num_eval_eps=num_eval_eps,compare=compare,compare_ah_idx=compare_ah_idx,reacher_sd=reacher_sd,acrobot_sd=acrobot_sd,min_trans=min_trans) #pylint: disable=E0632
...
def plot_eval_eps(seed,observ,env,idx,compare,pre_suf=''):
    if env.env_name=='ah':
        ...
        if env.with_obs:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_obs_idx_'+str(env.obs_idx)+goal_loc_suffix+'_'+env.env_name+'_'+str(idx)+pre_suf+suffix+'_compare.png',dpi=200)
        else:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_obs_idx_20'+goal_loc_suffix+'_'+env.env_name+'_'+str(idx)+pre_suf+suffix+'_compare.png',dpi=200)
        ...
        plt.legend()
        if not compare:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_'+env.env_name+'_'+str(idx)+pre_suf+'_not_compare.png',dpi=200)
        else:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_'+env.env_name+'_'+str(idx)+pre_suf+'_compare.png',dpi=200)
        ...
        plt.legend()
        if not compare:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_goal_height_'+str(env.goal_height)+'_'+env.env_name+'_'+str(idx)+pre_suf+'_not_compare.png',dpi=200)
        else:
            plt.savefig('./ppo2_free_results/eval/Eval_model_seed_'+str(seed)+'_goal_height_'+str(env.goal_height)+'_'+env.env_name+'_'+str(idx)+pre_suf+'_compare.png',dpi=200)
    else:
