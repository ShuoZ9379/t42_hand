...
def get_sai_cosai(gt_states_for_plot):
    ...
    return torch.cat((sai,cosai),dim=1)

def main(num_test,env_name,lr,nodes,model_ep,data_file_suffix='train',model_name_suffix='',seed=0):
    dropout_rate = .0
    ...
    for i in range(len(test_ds_ls)):
        ...
        if env_name=='Reacher-v2':
            ...
            gt_states_for_plot=get_sai_cosai(gt_states_for_plot)
            pred_states=get_sai_cosai(pred_states)
        else:
            raise Exception('Not implemented!')
