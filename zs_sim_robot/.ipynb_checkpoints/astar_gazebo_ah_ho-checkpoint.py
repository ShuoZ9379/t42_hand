...
model_path='/Users/zsbjltwjj/'+loccc+'/zs_sim_robot/trans_model_data/sim_cont_trajT_bs512_model512_BS64_loadT.pkl'
norm_path='/Users/zsbjltwjj/'+loccc+'/zs_sim_robot/trans_model_data/gazebo_ah_normalizaion/normalization_arr_sim_cont_trajT_bs512_model512_BS64_loadT_py2'
...
obs_set_path='/Users/zsbjltwjj/Downloads/t42_hand/beliefspaceplanning/rollout_node/set/set20c/'
set_mode='astar'
...
with open('/Users/zsbjltwjj/'+loccc+'/beliefspaceplanning/rollout_node/set/obs_20.pkl', 'rb') as f: 
...
inx = [0, 7, 8, 15, 2]
#inx = [0]
#Goal = np.array([[25-4, 104+19 ]])
j = 1
for i in inx:
    ctr = Goal[i]
    goal_plan = plt.Circle((ctr[0], ctr[1]), 5., color='m')
...
def mplot_planned_traj(num_steps,set_mode,set_path,initial_state,run_idx,goal_idx,plan_path,goal_loc,H1,H2,Obs,obs_dist,big_goal_radius=8):
    fig, ax = plt.subplots(figsize=(8,3.5))
    H = np.concatenate((np.array(H1)[:,:], H2), axis=0)
    pgon = plt.Polygon(H, color='y', alpha=1, zorder=0)
    ax.add_patch(pgon)
...
if set_idx.find('20c'):
    ...
    big_goal_radius=8
for mode in modes:
    obs_set_path='/Users/zsbjltwjj/Downloads/t42_hand/beliefspaceplanning/rollout_node/set/set20c'+mode+'/'
    if not os.path.exists(obs_set_path):
        os.makedirs(obs_set_path)
    obs_dist=0.75
    ac_weight=10000
